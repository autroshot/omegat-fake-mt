import { convertTagToApostrophe } from './text';

test('convertTagToApostrophe 함수', () => {
  expect(
  ...
  expect(convertTagToApostrophe(`<g1>server</g1> <-> <g2>client</g2>`)).toBe(
    `'server' <-> 'client'`
  );
  expect(convertTagToApostrophe(`<g1><div></g1> is tag.`)).toBe(
    `'<div>' is tag.`
  );
});
